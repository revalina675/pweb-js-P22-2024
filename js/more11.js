document.addEventListener("DOMContentLoaded", async () => {
  let quantity = 1;
  const defaultPricePerItem = 300000; // Harga tetap per item dalam Rupiah
  ...
  async function fetchProductData() {
    try {
      const response = await fetch("https://dummyjson.com/products/11"); // Mengambil data produk dengan ID 11
  ...
  addToCartButton.addEventListener("click", () => {
    ...
    const existingItem = cart.find((item) => item.id === 11); // ID produk
