document.addEventListener("DOMContentLoaded", async () => {
  let quantity = 1;
  const defaultPricePerItem = 149000; // Harga tetap per item dalam Rupiah
  ...
  async function fetchProductData() {
    try {
      const response = await fetch("https://dummyjson.com/products/2"); // Mengambil data produk dengan ID 2
  ...
  addToCartButton.addEventListener("click", () => {
    ...
    const existingItem = cart.find((item) => item.id === 2); // ID produk (dalam kasus ini adalah ID 2)
