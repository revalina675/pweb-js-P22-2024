document.addEventListener("DOMContentLoaded", async () => {
  let quantity = 1;
  const defaultPricePerItem = 179000; // Harga tetap per item dalam Rupiah
  ...
  async function fetchProductData() {
    try {
      const response = await fetch("https://dummyjson.com/products/6"); // Mengambil data produk dengan ID 6
  ...
  addToCartButton.addEventListener("click", () => {
    ...
    const existingItem = cart.find((item) => item.id === 6); // ID produk (dalam kasus ini adalah ID 6)
