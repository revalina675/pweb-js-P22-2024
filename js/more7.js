document.addEventListener("DOMContentLoaded", async () => {
  let quantity = 1;
  const defaultPricePerItem = 289000; // Harga tetap per item dalam Rupiah
  ...
  async function fetchProductData() {
    try {
      const response = await fetch("https://dummyjson.com/products/7"); // Mengambil data produk dengan ID 7
  ...
  addToCartButton.addEventListener("click", () => {
    ...
    const existingItem = cart.find((item) => item.id === 7); // ID produk (dalam kasus ini adalah ID 7)
