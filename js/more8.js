document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function fetchProductData() {
    try {
      const response = await fetch("https://dummyjson.com/products/8"); // Mengambil data produk dengan ID 8
  ...
  addToCartButton.addEventListener("click", () => {
    ...
    const existingItem = cart.find((item) => item.id === 8); // ID produk (dalam kasus ini adalah ID 8)
