document.addEventListener("DOMContentLoaded", async () => {
  let quantity = 1;
  const defaultPricePerItem = 350000;
  ...
  async function fetchProductData() {
      try {
          const response = await fetch("https://dummyjson.com/products/8");
  ...
  addToCartButton.addEventListener("click", () => {
      ...
      const existingItem = cart.find(item => item.id === 8);
      ...
      if (existingItem) {
          ...
          cart.push({
              id: 8,
              name: "Nana Jeans Brand Bradburry",
